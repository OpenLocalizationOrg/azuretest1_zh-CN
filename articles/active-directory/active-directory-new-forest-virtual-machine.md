...
1. 在经典的门户中，单击**新建** > **计算** > **虚拟机** > **剪辑库中**。 使用下列值来完成向导。 接受设置的默认值，除非另一个值是建议或要求。
    ...
    **虚拟机配置**  | <p>云服务︰ 选择<b>创建一个新的云服务</b>为第一个虚拟机，然后选择相同的云服务名称创建多个虚拟机将承载 DC 角色。</p><p>云服务 DNS 名称︰ 指定一个全局唯一名称</p><p>地区/关系组/虚拟网络︰ 指定的虚拟网络名称 （例如 WestUSVNet)。</p><p>存储帐户︰ 选择<b>所使用的自动生成的存储帐户</b>为第一个虚拟机，然后选择该相同的存储帐户名在创建多个虚拟机将承载 DC 角色。</p><p>可用性设置︰ 选择<b>创建可用性组</b>。</p><p>可用性设置名称︰ 键入可用性设置在创建第一个虚拟机，然后选择时，相同的名称，当您创建多个虚拟机的名称。</p>
    **虚拟机配置**  | <p>选择<b>安装虚拟机代理</b>和其他所需的扩展。</p>
2. 将磁盘连接到每个虚拟机将运行的 DC 的服务器角色。 需要额外的磁盘来存储 AD 数据库、 日志和 SYSVOL。 指定的大小 （如 10 GB) 的磁盘，并将**主机缓存首选项**设置为**无**。 有关的步骤，请参阅[如何将附加到 Windows 虚拟机数据磁盘](../storage-windows-attach-disk.md)。
