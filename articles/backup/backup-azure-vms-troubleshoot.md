...
| 备份 | 无法与快照状态的 VM 代理进行通信。 快照的虚拟机子任务超时。 -请请参阅有关如何解决此问题的故障排除指南。 | 如果 VM 代理问题或以某种方式阻止 Azure 的基础结构的网络访问，则会引发此错误。 <ul><li>了解有关[调试 VM 代理问题](#vm-agent) <li>了解有关[调试网络问题](#networking) </ul><br>如果 VM 代理不会造成任何问题，然后重新启动虚拟机。 有时 VM 状态不正确可能会导致问题并重新启动虚拟机将重置此"不良状态" |
| 备份 | 备份失败，发生内部错误-请重试该操作几分钟的时间。 如果问题仍然存在，请联系 Microsoft 技术支持 | 您可以获取此错误的原因︰ <ol><li> 没有要复制的数据太多。 <li>原始的虚拟机已被删除，因此无法执行备份。 为了保留已删除的虚拟机的备份数据但停止备份错误、 取消保护虚拟机并选择保存数据的选项。 这将停止备份日程安排以及定期的错误消息。 |
...
| 备份 | 负载平衡器配置虚拟机的备份不受支持。 | 无 |
| 备份 | 不支持使用多个网卡的虚拟机的备份。 | 无 |
...
| 取消作业 | 该作业不是可取消的状态-请稍候，直到完成作业。 <br>运算，在指定数据点中设置指定位数。<br> 所选的作业不是可取消的状态-请等待作业完成。| 在所有可能性中作业已基本完成;请稍候，直到完成作业 |
...
| 操作 | 错误详细信息 | 解决方法 |
| -------- | -------- | -------|
| Restore | 还原失败，云内部错误 | <ol><li>要还原的云服务配置了 DNS 设置。 您可以检查 <br>$deployment = 获取 AzureDeployment 服务"服务名"-插槽"生产"获得 AzureDns-DnsSettings $deployment。DnsSettings<br>如果没有配置地址，这意味着配置了 DNS 设置。<br> <li>要还原到的云服务配置了 ReservedIP，在云服务中的现有虚拟机处于停止状态。<br>您可以检查云服务具有通过使用以下 powershell cmdlet 的保留 IP:<br>$deployment = 获取 AzureDeployment 服务"服务名"-插槽"生产"$ dep。ReservedIPName</ol> |
| Restore | 所选的 DNS 名称已被使用-请指定一个不同的 DNS 名称并再试一次。 | 此处输入的 DNS 名称指的是云服务名称 (通常结尾。 cloudapp.net)。 这必须是唯一的。 如果您遇到此错误，您需要在还原过程中选择不同的虚拟机名称。 <br><br> 请注意，只对 Azure 的门户网站的用户显示此错误。 通过 PowerShell 还原操作将会成功，因为它只还原磁盘，并且不会创建虚拟机。 当 VM 明确由您创建磁盘还原操作后，将面临错误。 |
| Restore | 指定的虚拟网络配置不正确-请指定一个不同的虚拟网络配置，然后重试。 | 无 |
| Restore | 指定的云服务使用保留的 IP，并不与正在还原的虚拟机的配置相匹配-请指定不同的云服务不使用保留的 IP，或选择另一个要从中进行恢复的恢复点。 | 无 |
| Restore | 云服务已达到限制输入终结点的数量-通过指定不同的云服务或通过使用现有的终结点来重试此操作。 | 无 |
| Restore | 备份存储库和目标存储帐户处于两个不同的区域-确保在还原操作中指定的存储帐户处于同一个 Azure 地区作为备份存储库。 | 无 |
| Restore | 存储帐户指定为恢复操作不受支持-仅基本/标准存储帐户具有本地冗余或支持地理冗余复制设置。 请选择受支持的存储帐户 | 无 |
| Restore | 指定用于还原操作的存储帐户的类型不是在线-确保在还原操作中指定的存储帐户处于联机状态 | 这可能由于 Azure 存储中或由于中断的暂时性错误。 请选择另一个存储帐户。 |
| Restore | 已达到资源组配额-请从预览门户或联系人 Azure 支持增加限制删除某些资源组。 | 无 |
| Restore | 选定的子网不存在-请选择一个存在的子网 | 无 |
